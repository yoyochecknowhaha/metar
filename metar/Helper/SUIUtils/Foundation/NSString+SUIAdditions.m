...
// 时间戳转换为日期字符串
+ (NSString *)convertTimestampToDateString:(NSTimeInterval)timestamp {
...
#pragma mark - 设置行间距
+ (NSMutableAttributedString *)attributedString:(NSString *)string lineSpace:(CGFloat)lineSpace font:(CGFloat)font textAlignment:(NSTextAlignment)textAlignment {
    // 设置描述行间距
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:string];
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    [paragraphStyle setLineSpacing:lineSpace];//你所要设置的行距
    [paragraphStyle setAlignment:textAlignment];
    //调整行间距
    [attributedString addAttribute:NSParagraphStyleAttributeName
                                 value:paragraphStyle
                                 range:NSMakeRange(0, string.length)];
    [attributedString addAttribute:NSFontAttributeName value:gFont(font) range:NSMakeRange(0, string.length)];
